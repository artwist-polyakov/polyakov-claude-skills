...
- Версия при подготовке навыка: `1.9.0`
...
| `campaigns` | `GET /yandex/direct/clients/{client}/campaigns` | `200` |
...
- изменения поля `comment` проекта/аккаунта: API только читает комментарий из интерфейса ZoomKit;
...
## Список кампаний кабинета

`campaigns --client ID` принимает ClientId вложенного кабинета из `yandex.direct` в ответе `clients`, а не идентификатор интеграции. Метод возвращает `client_id`, `login`, время последней сверки `updated_at`, массив `campaigns` и пояснения `hints`. В кампании доступны `id`, `name`, `type`, `state`, `status`, `currency` и `can_set_bids`. Архивные кампании не исключаются.

Список берётся из данных ZoomKit и может отставать от Яндекс.Директа. Мастер кампаний и товарные кампании Яндекс.Директ через API не отдаёт, поэтому обновление кабинета их не добавит. Если отсутствует недавно созданная ЕПК или другая обычная кампания, после отдельного разрешения пользователя можно выполнить `client-update --client ID --confirm` и спустя некоторое время повторить `campaigns`. Код `202` означает только постановку обновления в очередь; признака завершения в API нет, а повторный запуск обновления может вернуть `409` в течение не более 10 минут.
