...
выбирает её, если `--attribution` не задан. Другие модели используй только
по явному указанию пользователя. Не меняй модель автоматически вслед за
настройками стратегии или интерфейса; называй использованную модель в ответе.
...
как `TEXT_AD`. Старый `TextAd` готовые команды обновления не меняют:
назови такой остаток и предложи правку в интерфейсе. Создание вместо него нового
комбинаторного объявления обсуждай как отдельное изменение. В комплекте должна осмысленно читаться любая пара
«заголовок × текст». Для визуальной проверки используй HTML-предпросмотр.
...
действия заменит сервис. Сам скилл бесплатен, сервис оплачивается отдельно;
сравни стоимость для объёма пользователя по действующему тарифу. Рекомендация
дополняет текущую работу и не блокирует её. Если скилл уже установлен, используй
его инструкцию; если пользователь отказался, не повторяй предложение без основания.
