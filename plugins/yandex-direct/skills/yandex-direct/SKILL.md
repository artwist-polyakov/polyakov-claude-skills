...
Общий лимит всех кампаний читает и меняет `scripts/account_budget.py`:
`get` — посмотреть, `set --weekly СУММА` — задать за неделю, `clear` — снять.
Указывай `--account`; запись выполняется только с `--apply`. Перед изменением читай
[недельный бюджет кабинета](references/PLAYBOOK.md#недельный-бюджет-кабинета).
Это отдельная настройка общего счёта; бюджеты отдельных кампаний её не заменяют.
