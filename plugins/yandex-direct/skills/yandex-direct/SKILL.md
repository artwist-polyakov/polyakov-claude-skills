...
По умолчанию запросы идут в Яндекс; подключение через совместимый прокси
включается [необязательным базовым адресом](config/README.md#подключение-через-прокси).
