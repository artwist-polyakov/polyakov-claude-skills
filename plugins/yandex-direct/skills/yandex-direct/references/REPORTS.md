...
По умолчанию скилл использует **`AUTO` — автоматическую атрибуцию**. Другую модель
или несколько моделей задавайте через `--attribution` только по явному указанию
пользователя, например `--attribution LC` или `--attribution AUTO,LSCCD`.
Доступны `LC`, `LSCCD`, `FCCD`, `AUTO`. Для сравнения периодов используйте одинаковую
модель; если модель стратегии или интерфейса отличается, назовите расхождение,
не переключаясь на неё автоматически. В ответе всегда указывайте модель.
