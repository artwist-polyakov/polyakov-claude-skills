...
Чтобы найти дополнения у `ShoppingAd` и `ListingAd`, читай объявления по ID,
...
Для нового объявления порядок тот же: создай комбинаторное объявление через
`ads_write.py ad create` или товарное через `shopping.py add`
([фиды и товарные объявления](FEEDS.md)), затем привяжи дополнения
...
Сохранённые дополнения не обязательно показываются в каждом месте показа,
включая товарную галерею. Проверяй нужный формат в предпросмотре Директа;
наличие привязки в API подтверждает настройку, а не её показ в каждой карточке.
