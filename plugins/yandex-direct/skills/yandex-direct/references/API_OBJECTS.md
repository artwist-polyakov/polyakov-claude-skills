...
| Значение | Пишется командой | Проверка результата |
|---|---|---|
| `SearchResults` | да | Поле читается и пишется, сверка после записи полная |
| `ProductGallery` | да | То же |
| `DynamicPlaces` | нет | При создании принимает значение `SearchResults`; для обновления это правило не обещано. Читается через API, отдельное управление — в интерфейсе |
| `Maps` | да, в обе части стратегии ЕПК | Поисковое поле перечитывается, сетевое отмечается `unchecked`: полного чтения API не предоставляет. Проверить переключатель в интерфейсе |
| `SearchOrganizationList` | да, в ЕПК | Поле API перечитывается; весь переключатель с отелями и галереей услуг проверить в интерфейсе |

Выбор охвата, пример «только галерея», явные настройки при создании и изменение
мест без перенастройки бюджета: [PLACEMENTS.md](PLACEMENTS.md). Пропуск поля
при создании не означает `NO`; при изменении неназванные поля сохраняются.
Параметры чтения `UnifiedCampaignSearchStrategyPlacementTypesFieldNames`
запрашиваются отдельно от `BiddingStrategy`.
