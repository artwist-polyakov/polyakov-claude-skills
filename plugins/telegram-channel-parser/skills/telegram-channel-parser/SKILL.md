...
- Некоторые посты Telegram не отдаёт через веб-превью. Если вместо текста приходит предложение открыть Telegram, поле текста в TSV/JSON содержит `[Текст недоступен в веб-превью Telegram: https://t.me/<channel>/<id>]`. Это пометка скилла, а не текст автора: сообщи о пропуске и дай ссылку, не пересказывай такой пост. Медиа без подписи остаётся с пустым текстом, если Telegram не пометил его как неподдерживаемое.
