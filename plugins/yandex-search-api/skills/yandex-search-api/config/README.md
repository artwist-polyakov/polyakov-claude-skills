...
| Выдержки страниц | Включены | Smart snippets: кусок страницы, а не только ссылка |
| Документов с выдержками | 20 | Максимум, который отдаёт API |

Выдержки (`search.smart_snippets`) — главная причина ставить этот скилл в
...
синхронном режиме. Выключаются флагом `--no-snippets` или
`"enabled": false` в конфиге — так дешевле, если нужны только позиции.
