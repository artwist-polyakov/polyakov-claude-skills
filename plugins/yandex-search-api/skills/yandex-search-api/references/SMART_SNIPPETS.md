...
> Ошибка в документации Яндекса, подтверждённая командой Search API: страница
> «Текстовый поиск» пишет «чтобы получить инфоконтекст, передайте заголовок
> `x-genesis-full-text`». Правильный ключ для инфоконтекстов —
> `x-genesis-info-context`, как на странице «Получение инфоконтекстов» и во
> всех её примерах. Проверено на живом API. Страницу обещали поправить.
>
> При этом `x-genesis-full-text`, похоже, существует как отдельный флаг: в
> ответе рядом с `info_context` приходит поле `full_text`, и на флаг
> инфоконтекстов оно пустое. Предположительно это запрос полного текста
> страницы вместо выдержки — но живым запросом мы это не проверяли, и в
> документации такой фичи нет. Скилл им не пользуется.
