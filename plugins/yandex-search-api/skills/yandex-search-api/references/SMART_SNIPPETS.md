...
> `x-genesis-full-text`». Правильный ключ — `x-genesis-info-context`, как на
> странице «Получение инфоконтекстов» и во всех её примерах. Страницу обещали
> поправить.
>
> `x-genesis-full-text` проверен живым запросом и **не делает ничего**: ответ
> приходит XML, инфоконтекстов ноль. Пустое поле `full_text`, которое видно в
> успешном ответе рядом с `info_context`, — рудимент, а не признак второй
> фичи. Перебирать ключи не нужно, правильный один.
