...
2. **Спроси пользователя** (если сайт не очевиден из контекста):
   ...
   `--host` и `hosts.sh --search` принимают полный кириллический домен
   (например, `хорошие-данные.рф`) или URL сайта (`https://хорошие-данные.рф/`).
   Домен автоматически преобразуется в IDNA средствами `python3`; схема и порт
   URL сохраняются. Латиница и готовые имена `xn--…` не требуют Python.
   Поиск по произвольной части кириллического имени не поддерживается:
   указывай полный домен. В кеше и результатах остаются исходные ASCII-адреса
   и `host_id` Яндекса.
