...
Правила: одна строка, без двойных кавычек и обратных слешей, до 200 символов. Название сохраняется дословно — то, что не пережило бы запись и чтение без искажений, скрипт отклоняет, а не переписывает. Если описание задачи занимает больше одной строки, а `--task-label` не передан — запуск завершается ошибкой, ничего не создаётся: назови задачу сам, а не рассчитывай, что скрипт угадает. При однострочном описании название берётся из него.
