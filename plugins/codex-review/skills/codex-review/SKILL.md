...
Каждый круг, начиная со второго, печатает в stderr строку
`Continuing task: <название> (previous round <время>)`. Если название не
совпадает с текущей задачей — цикл принадлежит другой работе: выбери один из
двух путей выше до отправки круга.
