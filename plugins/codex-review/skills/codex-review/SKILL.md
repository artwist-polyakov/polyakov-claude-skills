...
Опция работает для `init` и `code`. План передаётся через `--plan-file` — он читает файл так же. Одновременно с `--plan-file` или с описанием в аргументе — ошибка.
...
Правила: одна строка, без двойных кавычек, обратных слешей и управляющих символов, до 200 символов. Название сохраняется дословно — снимаются только обычные пробелы по краям, всё остальное, что не пережило бы запись и чтение без искажений, скрипт отклоняет, а не переписывает. Пустое значение в `--task-label` — ошибка. Если описание задачи занимает больше одной строки, а `--task-label` не передан — запуск завершается ошибкой, ничего не создаётся: назови задачу сам, а не рассчитывай, что скрипт угадает. При однострочном описании название берётся из него.
