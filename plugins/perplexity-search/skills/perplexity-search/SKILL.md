...
1. **Cache-first** — ключ кеша это само тело запроса, поэтому любой изменённый
   фильтр даёт новую запись. Идентичный запрос моложе `PPLX_CACHE_TTL` (900 c)
   не тратит деньги. `--no-cache` заставляет ходить в API.
